...
try:
    ...
    existing_info = None
    try:
        existing_info = pd.read_csv('Craigslist_Scrape.csv')
    ...
    try:
        ...
        if (not userInput == "quit"):
            ...
            while (endLoop):
                try:
                    craigslist_info.to_csv('Craigslist_Scrape.csv',index=False,
                        columns=['listing id','address','latitude','longitude','price','bedrooms','bathrooms','size',
                        'availability date',tags_names[0],tags_names[1],
